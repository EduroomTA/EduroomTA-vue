	...
	<section>
		...
		<sliding-header :threshold-hide="100" :threshold-open="200">
			...
			<template v-slot:second-header>
				<section style="top: 4rem !important; position: relative;">
						...
						<nav class="nav-dash in-scroll-page">
							<div class="container w60">
								<div class="nav-wrapper">
									...
									<ul class="nav-right">
										<router-link tag="li" :to="{ name: 'guru.kelas.mapel.diskusi.detail' }">
											<a class="primary-text" href="">Diskusi</a>
											<div class="nav-actives"></div>
										</router-link>
										<router-link tag="li" :to="{ name: 'guru.kelas.mapel.diskusi.rekap' }">
			...
			<div class="row nm mtb2">
				<div class="col s4">
					<label class="matter-textfield-outlined icon-input left-icon no-span" style="width: 100%">
						<input v-model="cari_nama" type="text" placeholder="Cari Siswa" content="Cari Nama Siswa" v-tippy='{ placement : "bottom" }'/>
						<span></span>
				...
				<div class="col s3">
					<label class="matter-textfield-flat center-select select-flat" style="width: 100%">
						<select v-model="cari_status" class="select">
							<option value="" selected>Semua Status</option>
							<option value="cccdd">Belum Berdiskusi</option>
							<option value="asdas">Sudah Berdiskusi</option>
						</select>
						<span></span>
					</label>
				</div>
					...
					<table>
						<thead>
							<tr>
								...
								<th>
									<a @click="sort('jumlah')" class="matter-button-text button-icon" content="Urutkan Berdasarkan Status" v-tippy='{ placement : "bottom" }'>
										<i 
						...
						<tbody>
							...
							<tr v-for="user in filteredUsers">
								...
								<td class="right-align">
									<div v-if="user.deskripsi === 'cccdd' && user.jumlah === 22233">
										<router-link 
										:to="{ name: 'guru.kelas.mapel.diskusi.kontribusi', params: { id: user.id} }" 
	...
	export default {
		...
		methods: {
			...
			filteredUsersByNama: function(users) {
				return users.filter(user => !user.nama.toLowerCase().indexOf(this.cari_nama.toLowerCase()))
			},
			filteredUsersByStatus: function(users){
				return users.filter(user => !user.deskripsi.indexOf(this.cari_status))
			},
			sort: function(s) {
