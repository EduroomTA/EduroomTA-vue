...
(function($){
  ...
  function _createTimepicker($input){
    ...
    hour_header.className = "text-center col s6 header-time";
    min_header.className = "text-center col s6 header-time";
    ok_button.className = "col s12 matter-button-text ok-button hide";
    // cancel_button.className = "col s6 btn btn-outline-secondary cancel-button";

    if (settings.use12HourClock){
      // if using 12-hour clock, widen the picker to 18rem
      // and create am/pm button
      $(picker).css("width", "19.2rem");
      hour_header.className = "text-center col s5";
      min_header.className = "text-center col s5";
      hour_select.className = "form-control col s5 hour-select";
      min_select.className = "form-control col s5 min-select";
      ...
      ampm_button_wrapper.className = "col s2 my-auto";
    }
    else{
      // default width of the picker is 12rem
      $(picker).css("width", "16rem");
      hour_select.className = "form-control col s6 hour-select";
      min_select.className = "form-control col s6 min-select";
    }
  ...
  function _bindTimepicker($picker, $input){
    ...
    $picker.find(".min-select").on("change", function(){
  ...
  function _showTimepicker($picker, $input){
    ...
    var right_pos = $input.offset()["right"];
    var top_pos = $input.offset()["top"] + $input.outerHeight() - 328 - $input.outerHeight();
    var top_pos2 = $input.offset()["top"] + $input.outerHeight();


    var scrollTop     = $(window).scrollTop(),
        elementOffset = $input.offset().top,
    ...
    if (distance > (328 + 15)) {
        ...
        $picker.offset({right: right_pos, top: top_pos});
        
    } else {
        $picker.insertAfter($input);
        $picker.offset({right: right_pos, top: top_pos2});
    }
