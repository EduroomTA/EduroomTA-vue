		...
		api: {
					...
					case 'popup':
						if ( opts.element.length ) {
							...
							var scrollTop     = $(window).scrollTop(),
    							elementOffset = opts.element.offset().top,
							...
							if (distance > (widgetSize.y + 15)) {
								widget.addClass('tip-below');
								widget.css('top', (offset.top - widgetSize.y) + 'px');
								
							} else {
								widget.addClass('tip-above');
								widget.css('top', (offset.top + elementSize.y) + 'px');
			...
			createDecadeView: function(opts, box) {
				...
				for (var i = 0, j = 0, d = 1; i < 16; i++, d++) {
					var row = rows[j],
						day = $('<div class="box-cell cell-day cell-year">'+ values[i] +'</div>');
					row.append(day);
					if (d > 3) {
						d = 0;
						j++;
					...
					if (i < 3 || i >= 16 - 3 || disabled) {
						...
						if ( date.toDateString() == today.toDateString() ) {
							day.addClass('cell-today cell-toyear');
						}
					}
			...
			createYearView: function(opts, box) {
				...
				for (var i = 0, j = 0, d = 1; i < 12; i++, d++) {
					var row = rows[j],
						day = $('<div class="box-cell cell-day cell-mont">'+ values[i].substring(0, 3) +'</div>');
					row.append(day);
					if (d > 3) {
						d = 0;
						j++;
					...
					// Today
					if ( date.toDateString() == today.toDateString() ) {
						day.addClass('cell-today cell-tomont');
					}
